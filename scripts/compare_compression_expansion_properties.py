...
parser = argparse.ArgumentParser(
    description=(
        "Compara el caso constant_properties con una prueba "
...
parser.add_argument(
    ...
    default=(
        ROOT
        / "docs/results/"
        "comparison_properties_compression_expansion.csv"
    ),
)
...
for cad in common_times:
    constant_properties_mean = baseline_means[cad]
    limited_mean = candidate_means[cad]

    constant_properties_extrema = baseline_extrema[cad]
    limited_extrema = candidate_extrema[cad]

    t_constant_properties = float(constant_properties_mean["T_mean_K"])
    t_limited = float(limited_mean["T_mean_K"])

    p_constant_properties = float(constant_properties_mean["p_mean_Pa"])
    p_limited = float(limited_mean["p_mean_Pa"])

    delta_t = t_limited - t_constant_properties
    delta_p = p_limited - p_constant_properties

    results.append(
        {
            "CAD": round(cad, 1),
            "T_mean_constant_properties_K": round(t_constant_properties, 6),
            "T_mean_variable_properties_K": round(t_limited, 6),
            "delta_T_mean_K": round(delta_t, 6),
            "delta_T_mean_pct": round(
                100 * delta_t / t_constant_properties,
                6,
            ),
            "p_mean_constant_properties_Pa": round(p_constant_properties, 3),
            "p_mean_variable_properties_Pa": round(
                p_limited,
                3,
            ),
            "delta_p_mean_Pa": round(delta_p, 3),
            "delta_p_mean_pct": round(
                100 * delta_p / p_constant_properties,
                6,
            ),
            "T_min_constant_properties_K": float(
                constant_properties_extrema["T_min_K"]
            ),
            "T_min_variable_properties_K": float(
                limited_extrema["T_min_K"]
            ),
            "T_max_constant_properties_K": float(
                constant_properties_extrema["T_max_K"]
            ),
            "T_max_variable_properties_K": float(
                limited_extrema["T_max_K"]
            ),
            "extreme_cells_constant_properties": int(
                constant_properties_extrema["extreme_cells"]
            ),
            "extreme_cells_variable_properties": int(
...
maximum_constant_properties_extremes = max(
    row["extreme_cells_constant_properties"]
    for row in results
)

maximum_limited_extremes = max(
    row["extreme_cells_variable_properties"]
    for row in results
)
...
print(
    ...
    f"constant_properties = {maximum_constant_properties_extremes}, "
    f"variable_properties = {maximum_limited_extremes}"
)